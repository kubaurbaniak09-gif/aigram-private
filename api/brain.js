...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (task === 'post_reactions') {
      ...
      const prompt = `Generate ${count} distinct AIgram comments to the player's post.\nPlayer: ${JSON.stringify(payload.player || {})}\nPost: ${JSON.stringify({ caption: payload.post?.caption, location: payload.post?.location })}\nAvailable personas: ${JSON.stringify(personas)}\n\nIMPORTANT REACTION ORDER:\n1. Inspect the attached image first and infer what is actually visible: subject, outfit, pose, expression, setting and overall vibe. Never invent a body detail that is not visible.\n2. Then read the caption/location and infer its exact intent.\n3. Combine BOTH sources. A caption about an accidental upload must not make every person joke about screenshots if the image itself invites a different reaction.\n\nFor an attractive, provocative or sensual adult photo, let a meaningful share of suitable personas react boldly to the person's appearance. Strong adult flirting, cheeky body compliments, desire, jealousy, teasing and mild profanity are welcome when consistent with that persona and relationship. Keep sexual language suggestive rather than graphically describing sex acts. Other personas can be funny, jealous, impressed, awkward, critical or neutral. Do not make everybody horny and do not make everybody nice.\n\nIf the caption asks a direct question, some comments should answer it directly, but the whole thread must not become 20 paraphrases of that answer. If travel/location is relevant, use it only when genuinely connected. Avoid generic filler such as repeated 'ładne zdjęcie', repeated screenshot/internet jokes, or comments that ignore the visible image. Make voices visibly different in length, punctuation, slang, emojis and attitude. Avoid duplicate openings and duplicate ideas.\nReturn an object exactly like {"comments":[{"botId":"id from personas","handle":"handle","text":"comment","chemistryDelta":0}]} with one comment per chosen persona.`;
